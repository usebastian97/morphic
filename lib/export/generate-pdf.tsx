/**
 * Client-side PDF generation for AgriEvidence responses.
 *
...
function stripMarkdown(text: string): string {
  return (
    text
      ...
      // Unordered lists
      .replace(/^[-*+]\s+/gm, '• ')
      // Spec blocks (AgriEvidence-specific fenced blocks)
      .replace(/```[\s\S]*?```/g, '')
...
function buildFilename(messageId: string): string {
  ...
  return `agri-evidence-${prefix}-${date}.pdf`
}
...
async function buildPdfDocument(data: MessageExportData) {
  ...
  return (
    <Document
      title={`AgriEvidence — ${data.userQuery.slice(0, 80)}`}
      author="AgriEvidence"
      subject="Agricultural Research Export"
    >
      <Page size="A4" style={styles.page}>
        {/* ── Header ─────────────────────────────────────── */}
        <View style={styles.header}>
          <Text style={styles.logoText}>AgriEvidence</Text>
          ...
          <>
            ...
            <View style={styles.sectionBlock}>
              ...
              <Text style={styles.scoreBreakdown}>
                {evidenceScore.breakdown.peer_reviewed_count} peer-reviewed ·{' '}
                {evidenceScore.breakdown.government_count} government ·{' '}
                {evidenceScore.breakdown.web_general_count} web general
                {evidenceScore.breakdown.used_fallback
                  ? ' · open-web fallback used'
                  : ''}
              </Text>
            </View>
          </>
        ...
        <View style={styles.footer} fixed>
          <Text style={styles.disclaimerText}>
            This document was generated by AgriEvidence. Content is based on
            AI-synthesised information from cited sources and is intended for
            informational purposes only. Always consult a qualified agronomist
            or extension officer before making farm management decisions.
          </Text>
          <Text
            style={styles.pageNumberText}
            render={({ pageNumber, totalPages }: { pageNumber: number; totalPages: number }) =>
              `Page ${pageNumber} of ${totalPages}  |  agri-evidence.com`
            }
          />
        </View>
      </Page>
